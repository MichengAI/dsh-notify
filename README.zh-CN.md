...
| 目标 | 操作 | 范围 |
| --- | --- | --- |
| 关闭插件 | 关闭「启用通知」。 | 全部弹窗和提示音 |
| 选择提示音 | 选择柔和 / 轻快 / 舒缓 / 清脆，再点「试听」。 | 由宿主播放 |
| 只关声音 | 关闭「播放提示音」。 | 仅提示音，Toast 仍会弹出 |
| 免打扰时段 | 打开时段开关并填写起止时间，支持跨午夜。 | Toast 和提示音 |
| 跟随系统勿扰 | 打开「跟随系统勿扰」。 | 专注助手开启时的提示音 |
| 完成提醒样式 | 选择「弹窗 + 提示音」或「仅角标」。 | 根 Agent 完成事件 |
| 合并完成通知 | 保持「合并同类通知」开启，5 秒内的多次完成会合成一条。 | 完成 Toast |
...
| `/api/dsh-notify/config` 与 `/preview` | 仅同源设置页 | 仅同源设置页 | 仅本机 |
